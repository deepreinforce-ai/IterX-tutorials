# Task: FlashInfer Fused MoE Kernel Optimization
...
| Agent | H100 | B200 |
|-------|------|------|
| GPT 5.2 Pro - OpenEvolve[mlsys26-agent-baseline](https://github.com/flashinfer-ai/mlsys26-agent-baseline)) | 3.57x | 4.38x |
| Claude 4.6 Opus - OpenEvolve ([mlsys26-agent-baseline](https://github.com/flashinfer-ai/mlsys26-agent-baseline)) | 3.80x | 5.74x |
| **IterX** | **15.62x** | **14.84x** |

The `get_reward()` function returns a tuple of **`(reward, correctness_check, error_msg, details)`**:
- **`reward`**: Speedup ratio (`reference_latency / code_latency`) - higher is better
- **`correctness_check`**: Whether the kernel produces correct results
- **`error_msg`**: Empty string `""` on success, or error description on failure
- **`details`**: Additional evaluation details

The evaluation process:
1. Sends the submitted Triton kernel code to a Modal GPU (H100 or B200)
2. Runs [flashinfer-bench](https://pypi.org/project/flashinfer-bench/) to verify correctness and measure latency
3. Computes reward as `reference_latency / code_latency` (speedup factor)
...
Generate a Triton kernel optimized for B200 GPU for the problem defined below:
...
- Write clean, efficient Triton code optimized for B200 architecture
...
- Optimize for B200 GPU characteristics (memory hierarchy, compute units, etc.)
...
```
The reward is the Speedup Ratio, which means `reward = BASELINE_LATENCY / CODE_LATENCY`
```
...
flashinfer_bench/
├── README.md                              # This file
├── definition.json                        # MoE task definition (shapes, dtypes, reference code)
...
└── result-from-mlsys26-agent-baseline/    # Baseline and IterX results
    ...
    ├── B200_opus_4_6.py                   # Claude Opus result on B200
    ├── H100_iterx.py                      # IterX result on H100
    └── B200_iterx.py                      # IterX result on B200
```
...
2. Configure your [Modal](https://modal.com/) account and credentials
...
```bash
# Run on B200
MODAL_GPU=B200 python run_iterx.py --model DeepSeek-V3-thinking --sample_size 2

# Run on H100
MODAL_GPU=H100 python run_iterx.py --model DeepSeek-V3-thinking --sample_size 2
```

---

## Evaluate Codes from mlsys26-agent-baseline

You can evaluate the baseline results from [mlsys26-agent-baseline](https://github.com/flashinfer-ai/mlsys26-agent-baseline) using `eval_once.py`:
...
```bash
MODAL_GPU=B200 python eval_once.py --code init_code.py
```
