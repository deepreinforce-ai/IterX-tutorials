...
def run(
    ...
    # Count tokens per expert to avoid unnecessary work
    ...
    # Sort by descending token count for better GPU utilization
    expert_info.sort(key=lambda x: -x[3])

    # Adaptive threshold: fused kernels for small batches, cuBLAS for large
    FUSED_THRESHOLD = 56

    needs_cublas = any(info[3] > FUSED_THRESHOLD for info in expert_info)

    # Lazy dequantize hidden states only if cuBLAS path is needed
    A_dequant = None
    ...
    for le, ge, token_idx, Tk in expert_info:
        ...
        # ===== SwiGLU =====
        C = C_buf[:Tk]
        silu_nc = (I + 2048 - 1) // 2048
        fused_silu_mul_kernel[(Tk, silu_nc)](G1, C, Tk, I, 2048)
