...
from eval_guess_lyric import get_reward
...
def create_task():
    ...
    initial_code_path = os.path.join(os.path.dirname(__file__), "initial_guess.txt")
    ...
    response = requests.post(
        ...
        headers=headers,
        json={
            "task_name": "Guess the Lyric",
            "task_description": """**Goal:** Guess the hidden reference lyric.
...
**Things to Avoid:** None""",
            "reward_description": "Jaccard similarity: Both guessed and reference lyrics are lowercased with punctuation removed, then split into word sets. Similarity = |intersection| / |union| of word sets. Range [0.0, 1.0], higher is better.",
...
def step_3_get_reward(code):
    # Use the get_reward function from eval_guess_lyric.py
    try:
